...
export function BreakoutGame({ paused, onScoreChange }: ArcadeGameProps) {
  ...
  }, [paused, gameOver, gameWon, onScoreChange]);

  const handleRetry = () => {
    ball.current = { x: 300, y: 320, vx: 3, vy: -3 };
    paddleX.current = 260;
    bricks.current = Array.from({ length: 5 }, () => Array(10).fill(1));
    score.current = 0;
    onScoreChange(0);
    setGameOver(false);
    setGameWon(false);
  ...
  return (
    <div className="relative mx-auto">
      ...
      {gameWon && <GameOverlay status="won" score={score.current} onRetry={handleRetry} />}
      {!gameWon && gameOver && <GameOverlay status="lost" score={score.current} onRetry={handleRetry} />}
    </div>
  );
}
